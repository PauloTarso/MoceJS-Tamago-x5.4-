function Doodle() {
    ...
    var Platform = function (x, y, type) {
        ...
        if (type === 1) {
            isso.primeiraCor = '#AADD00'; /*Plataformas Verdes*/
            isso.segundaCor = '#698B22';
            isso.onColisao = function () {
    ...
    var GameLoop = function () {
        ...
        var pontos = "PONTOS:" + points;
        /*↓↓↓↓↓↓↓↓↓↓Caixa é responsiva a largura do texto↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓*/
        frameIMGwidth = ctx.measureText(pontos).width;
        frameIMGheight = 45;
        ctx.drawImage(frameIMG, -5, height - 40, frameIMGwidth + 35, frameIMGheight);
        ctx.globalCompositeOperation = "source-over";
        ctx.fillText(pontos, 10, height - 7);
    ...
    var GameOver = function () {
        ...
        s = setTimeout(function () {
            ...
            grd.addColorStop(1, "#e699cc");
            // Faz o gradiente
            ctx.fillStyle = grd;
            ctx.fillRect(0, 0, 320, 500);
            /*Alguns desenhos que eu havia feito,escolher,com ou sem!*/
            //ctx.fillRect(-100,-100,320,500);		
            //ctx.fillRect(0,100,120,200);		
            //ctx.fillRect(200,100,320,500);
            ctx.drawImage(base_image, -30, 0, 750, 560);
            ctx.fillStyle = "#CE6BFF";
            ctx.drawImage(frameIMG, width / 2 - 158, height / 2 - 250, 320, 280);
            ctx.font = "24pt Roboto";
            ctx.fillText("GAME OVER", width / 2 - 110, height / 2 - 170);
            ctx.fillText("PONTUAÇÃO:" + points, width / 2 - 125, height / 2 - 80);
    ...
    function mouse(e) {
        ...
        if ((pos.x >= 10 && pos.x < 10 + 120 && pos.y >= 340 && pos.y < 340 + 120) && !estado) {
            window.score = Math.floor(points / 10);
